"""Storage layer for artifact registry service with atomic writes and locking."""
...
from .models import StoreFile, ArtifactRecord
...
def resolve_core_platform_dir() -> Path:
    """Resolve the core platform directory using environment variable or relative path detection."""
...
def make_artifact_id(name: str, run_id: str = "") -> str:
    """Generate artifact ID from name and run_id."""
    import hashlib
    import uuid

    content = f"{name}:{run_id}:{datetime.utcnow().timestamp()}:{uuid.uuid4().hex}"
