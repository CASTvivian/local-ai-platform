"""Storage layer for repo memory service with atomic writes and locking."""
...
from .models import StoreFile, RepoRecord, FixRecord, ContextSnapshot, KnowledgeEntry
...
def make_fix_id() -> str:
    ...
    content = f"fix:{datetime.utcnow().timestamp()}:{uuid.uuid4().hex}"
    h = hashlib.sha256(content.encode()).hexdigest()[:12]
    return f"fix_{h}"
...
def make_snapshot_id() -> str:
    ...
    content = f"snapshot:{datetime.utcnow().timestamp()}:{uuid.uuid4().hex}"
    h = hashlib.sha256(content.encode()).hexdigest()[:12]
    return f"snapshot_{h}"


def make_knowledge_id() -> str:
    """Generate knowledge ID."""
    import hashlib
    import uuid

    content = f"knowledge:{datetime.utcnow().timestamp()}:{uuid.uuid4().hex}"
    h = hashlib.sha256(content.encode()).hexdigest()[:12]
    return f"knowledge_{h}"
